...
def extract_shelf_type(user_input: str) -> str | None:
    ...
    #st.write(len(match))
    if len(match) == 0:
        st.error("No Valid Shelf Types detected. Please enter only one from (R2, R4, R6, R8)")
        st.stop()    
    elif len(match) > 1:
        st.error("Multiple Shelf Types detected. Please enter only one from (R2, R4, R6, R8)")
        st.stop()
